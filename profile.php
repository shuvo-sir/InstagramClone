...
if (!isset($_GET['curr_us']) || !isset($_GET['profile_for'])) {
    // Redirect to the login or home page with a message
    header("Location: login.php?error=missing_parameters");
    exit();
}

// Sanitize query parameters to prevent SQL injection
$curr_us = mysqli_real_escape_string($conn, $_GET['curr_us']);
$us = mysqli_real_escape_string($conn, $_GET['profile_for']);
?>
