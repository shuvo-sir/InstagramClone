    ...
    ?>
        ...
        <main class="feed">
            <?php
                ...
                MAX(posts.photo)                    AS photo, 
                MAX(posts.likes)                    AS likes,
                (
                    SELECT 1
                    FROM likes
                    WHERE likes.post_id = MAX(posts.post_id)
                        AND likes.likername = followings.username
                )                                   AS is_liked,
                MAX(posts.comments)                 AS comments,
                MAX(datediff(now(), posts.time_stamp)) AS time_stamp
                from followings
                join posts
                on posts.post_id = (
